# -*- coding: utf-8 -*-
"""
Figure 2: structure-function relationships
"""
...
from netneurotools import utils, datasets, stats, plotting
from scipy.stats import zscore, pearsonr, f, rankdata
from scipy.linalg import expm, eig
import seaborn as sns
from scipy.spatial.distance import squareform, pdist
from bct import distance, similarity
...
spins = stats.gen_spinsamples(coords, hemiid[cortex], n_rotate=10000, seed=1234)
nspins = 10000
eu = squareform(pdist(utils.get_centroids(cammoun[scale], image_space=True),
                      metric="euclidean"))
...
rsn_mapping = np.array(info.query('scale == @scale')['yeo_7'])
...
# exponential fit on receptor similarity
p0 = [1, -0.05, -0.1]  # initial parameter guesses
pars, _ = curve_fit(exponential, eu[cortex[:, None], cortex[None, :]][mask],
                    receptor_similarity[mask], p0=p0)
rs_reg = regress_dist(receptor_similarity[mask],
                      eu[cortex[:, None], cortex[None, :]][mask], pars)
parsF, _ = curve_fit(exponential, eu[cortex[:, None], cortex[None, :]][mask],
                     fc[mask], p0=p0)
fc_reg = regress_dist(fc[mask], eu[cortex[:, None], cortex[None, :]][mask], parsF)
# correlation with fc
...
sns.regplot(fc_reg, rs_reg, marker='.', scatter_kws={'s' : 5}, ax=ax3)
ax4.scatter(eu[cortex[:, None], cortex[None, :]][mask],
            receptor_similarity[mask], s=5)
...
ax4.set_title('y = ' + str(pars[0])[:4]
              ...
              + ' * x) + '+ str(pars[2])[:5])
plt.tight_layout()
plt.savefig(path+'figures/connectivity.eps')
...
for i in range(nnodes):
    ...
    x1 = sp[:, i]
    x2 = co[:, i]
    x3 = eu[cortex[:, None], cortex[None, :]][:, i]
    x4 = receptor_similarity[:, i]
...
plt.savefig(path+'figures/scatter_scfc_coupling.eps')


annot = datasets.fetch_cammoun2012('fsaverage')[scale]
brain = plotting.plot_fsaverage(data=rsq_r, order='rl', 
                                lhannot=annot.lh, rhannot=annot.rh,
                                colormap=cmap_seq, vmin=0, vmax=0.80,
                                views=['lat', 'med'],
                                data_kws={'representation': "wireframe"})
brain.save_image(path+'figures/surface_rsq_r.eps')

brain = plotting.plot_fsaverage(data=rsq_sc, order='rl',
                                lhannot=annot.lh, rhannot=annot.rh,
                                colormap=cmap_seq, vmin=0, vmax=0.80,
                                views=['lat', 'med'],
                                data_kws={'representation': "wireframe"})
brain.save_image(path+'figures/surface_rsq_sc.eps')

brain = plotting.plot_fsaverage(data=rsq_r - rsq_sc, order='rl',
                                lhannot=annot.lh, rhannot=annot.rh,
                                colormap=cmap_seq,
                                vmin=0, vmax=max(rsq_r - rsq_sc),
                                views=['lat', 'med'],
                                data_kws={'representation': "wireframe"})
brain.save_image(path+'figures/surface_scfc_coupling.eps')
